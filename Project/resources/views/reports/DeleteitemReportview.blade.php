            ...
            <div class="col-md-11">
                <div class="card">
                    <div class="card-header">
                        <h4 style="float: left;">Delete Items Report</h4>
                        <p>
                    ...
                    <div class="card-body">
                        ...
                        <thead>
                            ...
                            <tbody>
                                ...
                                @if( \Carbon\Carbon::parse($deleteitem->created_at)->format('Y-m-d') <= $edate && \Carbon\Carbon::parse($deleteitem->created_at)->format('Y-m-d') >= $sdate )
                                <tr>
                                    <td style="text-align: center;">{{ $deleteitem->userdata->name}}</td>
                                    <td style="text-align: center;">{{ $deleteitem->product->product_name}}</td>
                                    <td style="text-align: Center;">{{ $deleteitem->quantity}}</td>
                                    <td style="text-align: right;">{{ $deleteitem->price/$deleteitem->quantity }}</td>
...
<div class="modal">
    <div id="print">
        @include('printReport.DeleteitemReportprint')
    </div>
</div>
...
<script>
    function PrintReceptContent(el) {
        ...
        var myReceipt = window.open('', 'Delete_items_Report');
        myReceipt.screenX = 0;
        myReceipt.screenY = 0;
        ...
        myReceipt.document.write(`
            <html>
                <head>
                    <title>Delete Items Report</title>
                    <style>
